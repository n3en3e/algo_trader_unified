"""Build the Stage 4K-3 market data and contract qualification dry-run report."""
...
def run_stage4k3_market_data_contract_dry_run(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4k3_market_data_contract_dry_run_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4K-3 market data and contract qualification dry run requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4k2-plan-json", required=True)
    ...
    try:
        stage4k2_plan_report = json.loads(args.stage4k2_plan_json)
    ...
    readiness = report.get("readiness_for_stage4k4", {})
    return 0 if isinstance(readiness, dict) and readiness.get("ready_to_build_market_data_contract_execution_gate") is True else 1


def main(argv: list[str] | None = None) -> int:
    return run_stage4k3_market_data_contract_dry_run(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    market_data = report.get("market_data_dry_run_results", {})
    contract = report.get("contract_qualification_dry_run_results", {})
    lines = [
        "Stage 4K-3 market data and contract qualification dry run",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        f"ready_to_build_market_data_contract_execution_gate: {readiness.get('ready_to_build_market_data_contract_execution_gate') if isinstance(readiness, dict) else None}",
        ...
        f"market_data_provider_called: {market_data.get('provider_called') if isinstance(market_data, dict) else None}",
        f"contract_qualification_provider_called: {contract.get('provider_called') if isinstance(contract, dict) else None}",
    ]
