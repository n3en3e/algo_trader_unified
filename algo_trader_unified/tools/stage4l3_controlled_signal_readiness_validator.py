"""Build the Stage 4L-3 controlled signal-readiness validator report."""
...
def run_stage4l3_controlled_signal_readiness_validator(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4l3_controlled_signal_readiness_validator_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4L-3 controlled signal-readiness validator requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4l2-gate-json", required=True)
    ...
    try:
        stage4l2_gate_report = json.loads(args.stage4l2_gate_json)
    ...
    report = report_builder(
        stage4l2_signal_readiness_gate_report=stage4l2_gate_report,
        controlled_signal_readiness_validator=None,
    ...
    readiness = report.get("readiness_for_stage4l4", {})
    return 0 if isinstance(readiness, dict) and readiness.get("ready_to_build_signal_readiness_acceptance") is True else 1


def main(argv: list[str] | None = None) -> int:
    return run_stage4l3_controlled_signal_readiness_validator(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    execution = report.get("validator_execution_result", {})
    lines = [
        "Stage 4L-3 controlled signal-readiness validator",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        f"ready_to_build_signal_readiness_acceptance: {readiness.get('ready_to_build_signal_readiness_acceptance')}",
        f"selected_strategy_id: {selected.get('selected_strategy_id')}",
        f"operation_id: {operation.get('operation_id')}",
        f"validator_called: {execution.get('validator_called')}",
        f"validator_execution_success: {execution.get('success')}",
    ]
