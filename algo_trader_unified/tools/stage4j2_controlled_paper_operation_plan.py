"""Build the Stage 4J-2 controlled scheduled PAPER operation plan report."""
...
def run_stage4j2_controlled_paper_operation_plan(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4j2_controlled_paper_operation_plan_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4J-2 controlled PAPER operation plan requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4j1-readiness-json", required=True)
    parser.add_argument("--operation-window-config-json")
    ...
    try:
        stage4j1_readiness_report = json.loads(args.stage4j1_readiness_json)
        operation_window_config = _loads_optional(args.operation_window_config_json)
    ...
    readiness = report.get("readiness_for_stage4j3", {})
    return 0 if readiness.get("ready_to_build_controlled_paper_operation_dry_run") is True else 1


def main(argv: list[str] | None = None) -> int:
    return run_stage4j2_controlled_paper_operation_plan(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    window = operation_plan.get("proposed_operation_window", {}) if isinstance(operation_plan, dict) else {}
    lines = [
        "Stage 4J-2 controlled scheduled PAPER operation plan",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        "ready_to_build_controlled_paper_operation_dry_run: "
        f"{readiness.get('ready_to_build_controlled_paper_operation_dry_run')}",
        f"stage4j1_report_ready: {artifacts.get('stage4j1_report_ready')}",
        f"selected_strategy_id: {selected.get('selected_strategy_id')}",
        f"operation_id: {operation_plan.get('operation_id') if isinstance(operation_plan, dict) else None}",
        f"cadence: {window.get('cadence') if isinstance(window, dict) else None}",
    ]
