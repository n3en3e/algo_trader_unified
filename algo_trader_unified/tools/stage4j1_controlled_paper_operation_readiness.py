"""Build the Stage 4J-1 controlled scheduled PAPER operation readiness report."""
...
def run_stage4j1_controlled_paper_operation_readiness(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4j1_controlled_paper_operation_readiness_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4J-1 controlled PAPER operation readiness requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4i6-acceptance-json", required=True)
    ...
    try:
        stage4i6_acceptance_report = json.loads(args.stage4i6_acceptance_json)
    ...
    readiness = report.get("readiness_for_stage4j2", {})
    return 0 if readiness.get("ready_to_build_controlled_paper_operation_plan") is True else 1


def main(argv: list[str] | None = None) -> int:
    return run_stage4j1_controlled_paper_operation_readiness(
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    registry = report.get("registry_checks", {})
    lines = [
        "Stage 4J-1 controlled scheduled PAPER operation readiness",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        f"ready_to_build_controlled_paper_operation_plan: {readiness.get('ready_to_build_controlled_paper_operation_plan')}",
        f"stage4i6_report_ready: {artifacts.get('stage4i6_report_ready')}",
        f"stage4i_complete: {artifacts.get('stage4i_complete')}",
        f"selected_strategy_id: {selected.get('selected_strategy_id')}",
        f"selected_strategy_paper_eligible: {registry.get('selected_strategy_paper_eligible')}",
    ]
