"""Build the Stage 4J-4 controlled scheduled PAPER operation execution-gate report."""
...
def run_stage4j4_controlled_paper_operation_execution_gate(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4j4_controlled_paper_operation_execution_gate_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4J-4 controlled PAPER operation execution gate requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4j3-dry-run-json", required=True)
    ...
    parser.add_argument("--ack", action="append", default=None)
    args = parser.parse_args(argv)

    try:
        stage4j3_dry_run_report = json.loads(args.stage4j3_dry_run_json)
    ...
    readiness = report.get("readiness_for_stage4j5", {})
    return (
        0
        if readiness.get("ready_to_build_controlled_paper_operation_executor") is True
        else 1
    )


def main(argv: list[str] | None = None) -> int:
    return run_stage4j4_controlled_paper_operation_execution_gate(
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Stage 4J-4 controlled scheduled PAPER operation execution gate",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        "ready_to_build_controlled_paper_operation_executor: "
        f"{readiness.get('ready_to_build_controlled_paper_operation_executor')}",
        f"stage4j3_report_ready: {artifacts.get('stage4j3_report_ready')}",
        ...
        f"gate_available: {gate.get('available') if isinstance(gate, dict) else None}",
    ]
