"""Build the Stage 4K-4 market data and contract qualification execution-gate report."""
...
def run_stage4k4_market_data_contract_execution_gate(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4k4_market_data_contract_execution_gate_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4K-4 market data and contract qualification execution gate requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4k3-dry-run-json", required=True)
    ...
    try:
        stage4k3_dry_run_report = json.loads(args.stage4k3_dry_run_json)
    ...
    readiness = report.get("readiness_for_stage4k5", {})
    return 0 if isinstance(readiness, dict) and readiness.get("ready_to_execute_controlled_market_data_contract_providers") is True else 1


def main(argv: list[str] | None = None) -> int:
    return run_stage4k4_market_data_contract_execution_gate(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    market_data = report.get("market_data_execution_permissions", {})
    contract = report.get("contract_qualification_execution_permissions", {})
    lines = [
        "Stage 4K-4 market data and contract qualification execution gate",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        f"ready_to_execute_controlled_market_data_contract_providers: {readiness.get('ready_to_execute_controlled_market_data_contract_providers')}",
        f"selected_strategy_id: {selected.get('selected_strategy_id')}",
        f"operation_id: {operation.get('operation_id')}",
        f"may_call_controlled_market_data_provider_in_4k5: {market_data.get('may_call_controlled_market_data_provider_in_4k5')}",
        f"may_call_controlled_contract_qualification_provider_in_4k5: {contract.get('may_call_controlled_contract_qualification_provider_in_4k5')}",
    ]
