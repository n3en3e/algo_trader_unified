"""Build the Stage 4I-1 scheduled PAPER run readiness report."""
...
def run_stage4i1_scheduled_paper_run_readiness(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4i1_scheduled_paper_run_readiness_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4I-1 scheduled PAPER run readiness requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4h6-acceptance-json", required=True)
    ...
    try:
        stage4h6_activation_acceptance_report = json.loads(args.stage4h6_acceptance_json)
    ...
    readiness = report.get("readiness_for_stage4i2", {})
    return 0 if readiness.get("ready_to_build_first_scheduled_run_plan") is True else 1


def main(argv: list[str] | None = None) -> int:
    return run_stage4i1_scheduled_paper_run_readiness(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Stage 4I-1 scheduled PAPER run readiness",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        "ready_to_build_first_scheduled_run_plan: "
        f"{readiness.get('ready_to_build_first_scheduled_run_plan')}",
        f"stage4h6_report_ready: {artifacts.get('stage4h6_report_ready')}",
