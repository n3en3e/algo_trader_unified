"""Build the Stage 4I-4 scheduler/lifecycle activation gate report."""
...
def run_stage4i4_scheduler_lifecycle_activation_gate(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4i4_scheduler_lifecycle_activation_gate_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4I-4 scheduler/lifecycle activation gate requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4i3-dry-run-json", required=True)
    ...
    try:
        stage4i3_dry_run_report = json.loads(args.stage4i3_dry_run_json)
    ...
    readiness = report.get("readiness_for_stage4i5", {})
    return (
        0
        if readiness.get("ready_to_build_scheduler_lifecycle_activation_executor") is True
        else 1
    )


def main(argv: list[str] | None = None) -> int:
    return run_stage4i4_scheduler_lifecycle_activation_gate(
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    candidate = report.get("scheduler_lifecycle_activation_candidate", {})
    lines = [
        "Stage 4I-4 scheduler/lifecycle activation gate",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        "ready_to_build_scheduler_lifecycle_activation_executor: "
        f"{readiness.get('ready_to_build_scheduler_lifecycle_activation_executor')}",
        f"stage4i3_report_ready: {artifacts.get('stage4i3_report_ready')}",
