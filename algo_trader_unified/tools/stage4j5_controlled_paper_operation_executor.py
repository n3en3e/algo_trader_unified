"""Build the Stage 4J-5 controlled scheduled PAPER operation executor report."""
...
def run_stage4j5_controlled_paper_operation_executor(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4j5_controlled_paper_operation_executor_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4J-5 controlled PAPER operation executor requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4j4-gate-json", required=True)
    ...
    parser.add_argument("--allow-controlled-paper-operation-execution", action="store_true")
    args = parser.parse_args(argv)

    if not args.allow_controlled_paper_operation_execution:
        message = "ERROR: Stage 4J-5 refuses runner access without --allow-controlled-paper-operation-execution"
        if args.json_output:
            print(
                json.dumps(
                    {
    ...
    try:
        stage4j4_execution_gate_report = json.loads(args.stage4j4_gate_json)
    ...
    report = report_builder(
        ...
        selected_strategy_operation_runner=None,
        ...
        allow_controlled_paper_operation_execution=args.allow_controlled_paper_operation_execution,
    )
    if args.json_output:
        print(json.dumps(report, sort_keys=True))
    else:
        print(_format_human(report))
    readiness = report.get("readiness_for_stage4j6", {})
    return (
        0
        if readiness.get("ready_to_build_controlled_paper_operation_acceptance") is True
        else 1
    )


def main(argv: list[str] | None = None) -> int:
    return run_stage4j5_controlled_paper_operation_executor(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Stage 4J-5 controlled scheduled PAPER operation executor",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        "ready_to_build_controlled_paper_operation_acceptance: "
        f"{readiness.get('ready_to_build_controlled_paper_operation_acceptance')}",
        f"stage4j4_report_ready: {artifacts.get('stage4j4_report_ready')}",
        ...
        f"runner_called: {execution.get('runner_called') if isinstance(execution, dict) else None}",
    ]
