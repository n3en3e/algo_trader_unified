"""Build the Stage 4H-5 one-strategy paper activation executor report."""
...
def run_stage4h5_one_strategy_activation_executor(
    argv: list[str] | tuple[str, ...],
    *,
    report_builder: Callable[..., dict[str, Any]] = build_stage4h5_one_strategy_activation_executor_report,
) -> int:
    if "--dry-run-only" not in argv:
        print(
            "ERROR: Stage 4H-5 one-strategy activation executor requires --dry-run-only",
            file=sys.stderr,
        )
        return 1
    ...
    parser.add_argument("--stage4h4-gate-json", required=True)
    ...
    parser.add_argument("--allow-activation-write", action="store_true")
    parser.add_argument("--ack", action="append", dest="acknowledgements")
    args = parser.parse_args(argv)

    try:
        stage4h4_enablement_gate_report = json.loads(args.stage4h4_gate_json)
    ...
    report = report_builder(
        ...
        activation_writer=_CliUnavailableActivationWriter(),
        audit_writer=None,
        operator_acknowledgements=args.acknowledgements,
        allow_activation_write=args.allow_activation_write,
    ...
    readiness = report.get("readiness_for_stage4h6", {})
    return (
        0
        if readiness.get("ready_to_build_one_strategy_activation_acceptance_report") is True
        else 1
    )


def main(argv: list[str] | None = None) -> int:
    return run_stage4h5_one_strategy_activation_executor(sys.argv[1:] if argv is None else argv)
...
def _format_human(report: dict[str, Any]) -> str:
    ...
    lines = [
        "Stage 4H-5 one-strategy paper activation executor",
        f"success: {report.get('success')}",
        f"generated_at: {report.get('generated_at')}",
        "ready_to_build_one_strategy_activation_acceptance_report: "
        f"{readiness.get('ready_to_build_one_strategy_activation_acceptance_report')}",
        f"gates_passed: {gates.get('passed')}",
        f"selected_strategy_id: {selected.get('selected_strategy_id')}",
        f"activation_write_succeeded: {execution.get('activation_write_succeeded')}",
    ]
...
class _CliUnavailableActivationWriter:
    def activate_one_strategy(self, payload: dict[str, Any]) -> dict[str, Any]:
        raise RuntimeError("real activation writer execution is unavailable from the Stage 4H-5 CLI")
